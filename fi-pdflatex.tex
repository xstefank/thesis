...
A consensus protocol describes a series of steps that solve the consensus problem. These steps can be typically divided into three phases -- the selection of candidate values, the exchange of values between participants and the agreement. The final decision of each participant is irreversible. In transactions environment, the consensus represents the shared decision whether to commit or rollback the transaction. The following sections describe some of the mostly widely used consensus protocols that may be employed in (potentially distributed) transactional systems.

\subsection{2PC}

The \textit{Two phase commit protocol} is one of the most known employed consensus protocols used not only for the transaction processing. The procedure consists of two phases:
...
The main disadvantage is that the 2PC is a blocking protocol. When the participant sends its prepare message it will block until either commit or rollback message is received. In case of the coordinator failure, this may oppose significant problems.
...
%Fedora 27 x64, RAM 16GB, 6 vCPUs, SSD 320GB,
