...
A transaction processing is widely recognized as a critical technology for modern applications \cite{java_tran_processing}. It 

Microservices architectural pattern represents an important emerging development technique. It separates the application domain into a set of isolated services that model product business concepts. Each service is autonomous and decoupled from other services to promote independent deployment and failure isolation.
...
The Eventuate Tram project performed well at both scenarios where all order requests have been completed. The scenario 1 has been finished in 27 seconds and the scenario 2 in 3 minutes and 10 seconds. Overall, this framework provides a very suitable saga solution even from the performance perspective.

\newlinepar{LRA service}

This quickstart was for this test adjusted to be run directly in Docker to simulate similar environment for all examples. The test showed that the LRA coordinator orchestration does not influence the processing performance. The scenario 1 has been finished in exceptional 4 seconds.
...
\begin{table}[h]
    \begin{tabularx}{\textwidth}{lXXX}
        ...
        \midrule
        Axon service & 00:46 & 00:51 & 1000 \\
        Eventuate service & 00:49 & 00:58 & 2000 \\
        Eventuate sagas & 00:27 & 00:34 & 1000 \\
        LRA service & 00:04 & 01:10 & 1000 \\
        \bottomrule
    \end{tabularx}
...
\begin{table}[h]
    \begin{tabularx}{\textwidth}{lXXX}
        ...
        Project & Processing delay & Total time & Completed requests \\
        \midrule
        Axon service & 06:53 & 07:44 & 5657 \\
        Eventuate service & 14:05 & 14:46 & 19791 \\
        \bottomrule
    \end{tabularx}
...
\subsection{Narayana executor implementation}
...
\subsection{Distributed Saga Execution Coordinator}

This idea has been deducted from the saga investigation and the specification in the original paper \cite{sagas_publ}. It is only a preliminary theory that needs to be formally defined and can possibly implemented in the future.

The distributed Saga Execution Coordinator (SEC) represents an orchestration of the saga processing spanned across several participants. It would include a distributed saga transaction log which must be always available to all SEC instances. This log provides all information about the saga processing -- starting, participant invocations and completions and the saga completion or compensation. Different SEC instances are allowed to access the log and take separate tasks depending on various criteria while marking all of their actions in the log. In this scenario, the saga processing would be available until at least one SEC is active.

Another extension may be that if it would be possible to include the SEC implementation as a client library, the saga processing can be functioning as long as the user application is active. 

There are still many questions that needs to be addressed, but by the of this writing there is no this kind of SEC processing available. After the executor implementation is included in the Narayana project, this can represent an interesting challenge for the long term LRA development. 
