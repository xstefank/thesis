...
This section presents the current implementations of the saga pattern available for the enterprise use. The four explored frameworks are Axon \cite{axon_framework}, Eventuate \cite{eventuate.io}, Narayana Long Running Actions (LRA) \cite{narayana_lra} and Eventuate Tram \cite{eventuate-tram}.
...
The Eventuate ES provides application with the programming model based on the event sourcing. Event sourcing is a mechanism that tracks all changes to the data model as a sequence of events stored in the event log. Every model change is stored in the log and the sequence can be anytime replayed to restore the application state. This platform is also based on the CQRS pattern which allows natural employment of event sourcing capabilities for aggregate entities. It is provided in two versions -- as a service hosted on Amazon Web Services (AWS) or as open source local platform.
