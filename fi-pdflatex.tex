...
The current implementation of the \texttt{LRA executor extension} discussed in the previous chapters provides a working proof of concept of potential Narayana LRA execution capabilities. However, there are still some tasks that need to be addressed before it can be included in the product.

The first assignment would be the refactoring of the current LRA code base to extract the orchestration logic from the coordinator. This will allow to employ different communication patterns and protocols for the LRA processing on top of the same service layer.

One of the features of this solution is the asynchronous LRA processing provided directly by the framework. The prototype executes each asynchronous LRA in a new thread from the employed cached thread pool. Nevertheless, this is the simplest solution that may not be sufficient in the long term.
