...
The consensus problem represents the procedure of achieving the  agreement for the shared data value between several components. It has its application in many environments including transactions where the TM needs to conclude whether a distributed transaction can be committed depending on the participants consensus.

This problem is also generally known as \textit{The Byzantine Generals Problem} which denotes the agreement of the battle strategy that generals have to conclude. The general in this model may also act as a traitor and provide misleading information which represents a failed system component. The paper proved that the consensus can be achieved if less than one third of the generals are traitors.
