...
Atomicity is generally achieved by the usage of the consensus multi-phase protocols. Standardized protocol is the two phase commit protocol which is used by the majority of modern transaction systems including Narayana. Consensus protocols together with other approaches for obtaining the atomicity property will be investigated in the following section.
...
An isolation property takes effect when multiple transactions can be executed concurrently on the same resources. It means that concurrent transactions do not interfere one with another. Therefore each concurrent execution on the shared resource must be equivalent to some serial ordering of contained transactions which is why the isolation is often also referenced as serializability.
